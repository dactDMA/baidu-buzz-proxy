...
const items=document.getElementById('items'),result=document.getElementById('result'),cancel=document.getElementById('cancel');let rendered=false,currentStatus='',statusStarted=Date.now(),lastBytes=0,lastBytesAt=performance.now(),smoothedSpeed=0,measuringUpload=false;
