...
white-space:nowrap}progress{width:100%;height:14px;accent-color:var(--accent)}code{overflow-wrap:anywhere}
...
<p id="message" class="muted"></p><progress id="progress" max="1" value="0"></progress><p id="numbers" class="muted"></p>
...
const progress=document.getElementById('progress'),numbers=document.getElementById('numbers'),selection=document.getElementById('selection');
const items=document.getElementById('items'),result=document.getElementById('result'),cancel=document.getElementById('cancel');let rendered=false;
...
async function refresh(){{try{{const response=await fetch('/api/jobs/'+id);const data=await response.json();if(!response.ok)throw new Error(data.detail||'Could not load job');state.textContent=data.state.replaceAll('_',' ');message.textContent=data.status;error.textContent=data.error||'';progress.max=Math.max(data.total_bytes,1);progress.value=Math.min(data.transferred_bytes,data.total_bytes||1);numbers.textContent=data.total_bytes?bytes(data.transferred_bytes)+' / '+bytes(data.total_bytes):'';selection.hidden=data.state!=='awaiting_selection';if(data.state==='awaiting_selection')renderItems(data);result.hidden=data.state!=='completed';if(data.result_url)document.getElementById('download').href=data.result_url;cancel.disabled=['completed','failed','cancelled'].includes(data.state);if(!['completed','failed','cancelled','awaiting_selection'].includes(data.state))setTimeout(refresh,2000);}}catch(e){{error.textContent=e.message;setTimeout(refresh,5000);}}}}
